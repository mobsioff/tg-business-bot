...
import { findArchivedMessage, insertArchivedMessage, insertEditEvent, insertMessageMedia, markMessageDeleted, updateArchivedMessageText } from '../repositories/message.repo.js';
...
export async function archiveBusinessMessage(message) {
    ...
    for (const item of [message.document, message.video, message.voice, message.audio]) {
        if (!item)
            continue;
        const type = message.document === item ? 'document' :
...
export async function recordBusinessMessageEdit(message) {
    const connectionId = message.business_connection_id;
    if (!connectionId)
        return null;
    const chatId = String(message.chat.id);
...
export async function recordBusinessMessageDeletion(connectionId, chatId, messageId) {
