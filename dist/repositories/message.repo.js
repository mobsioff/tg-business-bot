...
export async function insertArchivedMessage(row) {
    const payload = {
...
export async function insertMessageMedia(row) {
    const payload = {
        connection_id: row.connectionId,
        chat_id: row.chatId,
        message_id: row.messageId,
...
export async function findArchivedMessage(connectionId, chatId, messageId) {
...
export async function markMessageDeleted(connectionId, chatId, messageId, deletedAtUnix) {
...
export async function insertEditEvent(row) {
    const payload = {
        connection_id: row.connectionId,
        chat_id: row.chatId,
        message_id: row.messageId,
...
export async function listRecentMessagesByConnection(connectionId, limit = 5) {
    const { data, error } = await supabase
        .from('archived_messages')
        .select('*')
        .eq('connection_id', connectionId)
