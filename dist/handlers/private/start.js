...
export async function handlePrivateStart(message) {
    if (!message.from)
        return;
    ...
    if (payload?.startsWith('bizChat')) {
        ...
        if (connection?.connection_id) {
            ...
            await sendMessage(message.chat.id, 'Я получил deep link из business-чата, но connection ещё не появился в базе. Обычно это происходит через пару секунд после фактического подключения в Telegram Business.');
        }
    }
}
