...
export async function maybeDownloadMedia(target: MediaTarget) {
  if (!env.SAVE_MEDIA) return { localPath: null, telegramFilePath: null };

  const file = await getFile(target.fileId);
  if (!file.file_path) return { localPath: null, telegramFilePath: null };
