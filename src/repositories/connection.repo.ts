...
export async function upsertBusinessConnection(connection: BusinessConnection) {
  const payload = {
...
export async function getLatestConnectionByOwnerChatId(ownerChatId: string) {
  const { data, error } = await supabase
    .from('business_connections')
    .select('*')
...
export async function upsertBusinessChat(connectionId: string, chat: TgChat) {
  const payload = {
